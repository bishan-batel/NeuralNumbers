...
public final class Malformer {
	...
	public static void randomShift(double[] data) {
		...
		int minX = ComplexDigits.IMAGE_RES;
		int minY = ComplexDigits.IMAGE_RES;
		...
		for (int i = 0; i < ComplexDigits.IMAGE_RES; i++) {
			for (int j = 0; j < ComplexDigits.IMAGE_RES; j++) {
				if (data[i * ComplexDigits.IMAGE_RES + j] > 0) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				packed[i * width + j] = data[(i + minY) * ComplexDigits.IMAGE_RES + j + minX];
			}
		}

		// now, randomly shift the image
		int newMinX = (int) (Math.random() * (ComplexDigits.IMAGE_RES - width));
		int newMinY = (int) (Math.random() * (ComplexDigits.IMAGE_RES - height));
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				data[(i + newMinY) * ComplexDigits.IMAGE_RES + j + newMinX] = packed[i * width + j];
			}
		}
	}
